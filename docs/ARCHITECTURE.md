...
用户输入
  ...
  ├── _compose_messages()   # [system] + [roles] + [knowledge] + [history]
  ├── _call_with_fallback() # 流式调用 LLM，失败时按回退链重试
  │   ├── 纯文本回复 → 直接输出
  │   └── 工具调用 → 执行工具并反馈结果，继续下一轮
  └── 历史追加
```
...
**SubAgentBase 工具调用循环（`execute()` 方法）：**

```
messages = [system_prompt, user_task]
for turn in range(30):
    resp = _call_llm(messages, tools=tool_definitions)
    if resp.tool_calls:
...
return {"success": False, "result": "超过最大轮次"}
```
