...
ChatAgent.__init__
  ...
  └── _init_sub_agents()      # 实例化 18 个 Sub-Agent 并注册到 Dispatcher
```
...
18 个专业 Sub-Agent，每个继承 `SubAgentBase`：
...
| `OptimizationAgent` | 多目标优化：敏感性分析、ARSM/EA（支持热-电磁联合优化） |
| `ReportingAgent` | 报告生成：HTML/PDF 双轨输出 |
| `EVPowertrainAgent` | EV 电驱系统联仿：电池+控制器+电机联合仿真 |
| `NVHAgent` | NVH 噪声振动：电磁力→结构振动→声学完整链路 |
| `CostAgent` | 电机成本估算：材料用量+制造工艺成本分析 |
...
- `OptimizationAgent`：识别 `optimization_study`、`sensitivity_study`、`parametric_sweep`、`coupled_optimization`
- `EVPowertrainAgent`：识别 `battery_setup`、`controller_setup`、`powertrain_cosimulation`、`powertrain_postprocess`
- `NVHAgent`：识别 `nvh_full_chain`、`em_force_extraction`、`nvh_modal`、`nvh_harmonic`、`nvh_noise`
- `CostAgent`：识别 `cost_estimation`、`cost_comparison`、`cost_optimization`、`price_inquiry`
...
210+ 工具按仿真域分模块，每个函数返回标准结构 `{"success": bool, "result": ...}`。
