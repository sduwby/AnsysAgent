...
ChatAgent.__init__
  ...
  └── _init_sub_agents()      # 实例化 10 个 Sub-Agent 并注册到 Dispatcher
```
...
10 个专业 Sub-Agent，每个继承 `SubAgentBase`：
...
110+ 工具按仿真域分模块，每个函数返回标准结构 `{"success": bool, "result": ...}`。
