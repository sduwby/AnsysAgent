...
def connect_mapdl(
    ...
    try:
        ...
        return _ok(f"已连接到 MAPDL {version}（{'本地' if launch_local else f'{server}:{port}'}）")
...
def run_thermal_stress_analysis(
    ...
    try:
        ...
        avg_temp = sum(d["temp"] for d in node_temps) / len(node_temps) if node_temps else ref_temp_C
